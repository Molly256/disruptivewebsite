...
export default function BindWalletPage() {
  ...
  const handleBindWallet = async () => {
    ...
    const data = await saveToDB({type: 'wallet', withdrawType, walletName, walletAddress})
    setMsg(data.error || t('walletBound'))
  }
        ...
        <div style={{ background: '#FFF', borderRadius: 12, padding: 16, marginBottom: 40 }}>
          ...
          {user.boundWallet &&!showForm && (
            <div style={{ background: '#F5F5F5', padding: 16, borderRadius: 12 }}>
              ...
              <button
                onClick={() => setShowForm(true)}
                style={{ width: '100%', background: '#FF0000', color: '#FFF', border: 'none', borderRadius: 12, padding: '12px', fontSize: 14, fontWeight: 700, cursor: 'pointer', marginTop: 12 }}
              >
                {t('edit')}
              </button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ background: '#F5F5F5', borderRadius: 12, padding: 14 }}>
                ...
                <select
                  ...
                  style={{ width: '100%', padding: '12px 14px', borderRadius: 8, border: '1px solid #DDD', fontSize: 15, background: '#FFF', fontWeight: 600 }}
                >
                  {networks.map(tw => (
                    <option key={tw} value={tw}>{tw} {withdrawType === tw && '✓'}</option>
                  ))}
                </select>
              ...
              <div style={{ background: '#F5F5F5', borderRadius: 12, padding: 14 }}>
                ...
                <input
                  ...
                  style={{ width: '100%', padding: '12px 14px', borderRadius: 8, border: '1px solid #DDD', fontSize: 15, boxSizing: 'border-box', marginBottom: 16 }}
                />
                ...
                <input
                  ...
                  style={{ width: '100%', padding: '12px 14px', borderRadius: 8, border: '1px solid #DDD', fontSize: 15, boxSizing: 'border-box' }}
                />
              </div>
              ...
              <button
                ...
                style={{ width: '100%', background: '#E0E0E0', color: '#000', border: 'none', borderRadius: 12, padding: '14px', fontSize: 16, fontWeight: 700, cursor: 'pointer' }}
              >
                {t('cancel')}
              </button>
            </div>
