...
<header style={{display: 'flex',justifyContent: 'space-between',alignItems: 'center',padding: '20px 40px',borderBottom: '1px solid #eee',position: 'sticky',top: 0,background: '#fff',zIndex: 100,height: '80px'}}>
<div style={{ maxWidth: '200px', height: '45px', display: 'flex', alignItems: 'center' }}>
<img src="/logo.png" alt="Disruptive Logo" style={{ width: '100%', height: 'auto', objectFit: 'contain' }} />
</div>
<button onClick={() => setMenuOpen(!menuOpen)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '10px' }} aria-label="Menu">
{menuOpen ? (<div style={{ fontSize: '40px', color: '#000', lineHeight: 1, fontWeight: '300' }}>×</div>) : (
...
{menuOpen && (
<div style={{position: 'fixed',top: 80,left: 0,width: '100%',height: 'calc(100vh - 80px)',background: '#fff',zIndex: 99,padding: '60px 40px',overflowY: 'auto'}}>
<nav style={{ maxWidth: '600px', margin: '0 auto' }}>
<div style={{ marginBottom: '45px' }}>
<div onClick={() => toggleDropdown('what')} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer'}}>
<span style={{ fontSize: '28px', fontWeight: '700', color: '#000', letterSpacing: '1px' }}>WHAT WE DO</span>
<span style={{fontSize: '24px',color: '#d40000',fontWeight: '300',transform: openDropdown === 'what' ? 'rotate(180deg)' : 'rotate(0deg)',transition: '0.3s'}}>⌄</span>
</div>
{openDropdown === 'what' && (<div style={{ marginTop: '25px' }}>
{['All Services','Paid Search','Paid Social','SEO','Amazon','Lifecycle Marketing','CRO','Creative','Data Analytics','Lead Generation'].map((item) => (
<div key={item} onClick={() => scrollTo('what-we-do')} style={{fontSize: '20px', color: '#333', fontWeight: '300', cursor: 'pointer', margin: '18px 0', paddingLeft: '10px'}}>{item}</div>))}
</div>)}
</div>
<div style={{ marginBottom: '45px' }}>
<div onClick={() => toggleDropdown('who')} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer'}}>
<span style={{ fontSize: '28px', fontWeight: '700', color: '#000', letterSpacing: '1px' }}>WHO WE HELP</span>
<span style={{fontSize: '24px',color: '#d40000',fontWeight: '300',transform: openDropdown === 'who' ? 'rotate(180deg)' : 'rotate(0deg)',transition: '0.3s'}}>⌄</span>
</div>
{openDropdown === 'who' && (<div style={{ marginTop: '25px' }}>
{['B2B','B2C','eCommerce','Local','Garage Door','SaaS','Online Education','Outdoor Sporting Goods','High AOV Brands'].map((item) => (
<div key={item} onClick={() => scrollTo('who-we-help')} style={{fontSize: '20px', color: '#333', fontWeight: '300', cursor: 'pointer', margin: '18px 0', paddingLeft: '10px'}}>{item}</div>))}
</div>)}
</div>
<div onClick={() => scrollTo('results')} style={{fontSize: '28px', fontWeight: '700', color: '#000', cursor: 'pointer', marginBottom: '45px', letterSpacing: '1px'}}>RESULTS</div>
<div style={{ marginBottom: '45px' }}>
<div onClick={() => toggleDropdown('about')} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer'}}>
<span style={{ fontSize: '28px', fontWeight: '700', color: '#000', letterSpacing: '1px' }}>WHO WE ARE</span>
<span style={{fontSize: '24px',color: '#d40000',fontWeight: '300',transform: openDropdown === 'about' ? 'rotate(180deg)' : 'rotate(0deg)',transition: '0.3s'}}>⌄</span>
</div>
{openDropdown === 'about' && (<div style={{ marginTop: '25px' }}>
{['About Us','Why Disruptive','Meet Our Team','Our Mission','Giving Back','Careers'].map((item) => (
<div key={item} onClick={() => scrollTo('who-we-are')} style={{fontSize: '20px', color: '#333', fontWeight: '300', cursor: 'pointer', margin: '18px 0', paddingLeft: '10px'}}>{item}</div>))}
</div>)}
</div>
<div style={{ marginBottom: '50px' }}>
<div onClick={() => toggleDropdown('resources')} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer'}}>
<span style={{ fontSize: '28px', fontWeight: '700', color: '#000', letterSpacing: '1px' }}>RESOURCES</span>
<span style={{fontSize: '24px',color: '#d40000',fontWeight: '300',transform: openDropdown === 'resources' ? 'rotate(180deg)' : 'rotate(0deg)',transition: '0.3s'}}>⌄</span>
</div>
{openDropdown === 'resources' && (<div style={{ marginTop: '25px' }}>
{['General','Blog','Partners','Webinars/Trainings','Reviews','Contact Us'].map((item) => (
<div key={item} onClick={() => scrollTo('resources')} style={{fontSize: '20px', color: '#333', fontWeight: '300', cursor: 'pointer', margin: '18px 0', paddingLeft: '10px'}}>{item}</div>))}
</div>)}
</div>
<button onClick={() => scrollTo('contact-us')} style={{marginTop: '20px',padding: '18px 40px',background: 'linear-gradient(180deg, #e60000 0%, #b30000 100%)',color: '#fff', border: 'none',fontSize: '16px', fontWeight: '700', letterSpacing: '2px', cursor: 'pointer'}}>LET'S TALK</button>
</nav>
</div>)}
...
<span style={{ color: '#d40000', fontWeight: '700' }}>RISK-FREE</span><span style={{ color: '#d40000', fontWeight: '400' }}> GUARANTEE</span>
</p>
<h2 style={{ fontSize: '36px', fontWeight: '900', color: '#000', lineHeight: '1.2', marginBottom: '16px' }}>Get Results in 90 Days—<br />Or You Don’t Pay</h2>
<div style={{ width: '48px', height: '2px', background: '#d40000', marginBottom: '32px' }}></div>
<p style={{ fontWeight: '700', color: '#000', marginBottom: '16px' }}>Most Agencies Guess. We Audit, Prove, and Guarantee.</p>
<p style={{ color: '#333', marginBottom: '24px', lineHeight: '1.6' }}>Our free marketing audit is your first step to identifying wasted spend and missed opportunities. Whether you hire us or not, you’ll walk away with a clear roadmap to grow faster and smarter.</p>
<p style={{ color: '#333', marginBottom: '24px', lineHeight: '1.6' }}><span style={{ textDecoration: 'underline' }}>For qualifying brands</span>, we guarantee measurable growth within 90 days—or your money back. No fluff. No long-term contracts. Just real results.</p>
<button onClick={() => scrollTo('contact-us')} style={{width: '100%',background: 'linear-gradient(180deg, #dc2626 0%, #b91c1c 100%)',color: '#fff',fontWeight: '700',padding: '10px 0',borderRadius: '0',border: 'none',fontSize: '16px',letterSpacing: '0.05em',cursor: 'pointer',textTransform: 'uppercase',display: 'flex',flexDirection: 'row',justifyContent: 'center',alignItems: 'center',gap: '6px'}}>
...
<h2 style={{fontSize: '48px',fontWeight: '900',textTransform: 'uppercase',letterSpacing: '2px',margin: '0 0 50px 0'}}>HUNDREDS OF CLUTCH REVIEWS</h2>
...
<p style={{fontSize: '16px',fontWeight: '400',color: '#000',lineHeight: '1.7',maxWidth: '900px'}}>
<span style={{ textDecoration: 'underline' }}>Disruptive Advertising has been rated as the best performance marketing agency for authentic brands and marketers.</span> We elevate the way business is done by pairing empowered marketers with win-win-win minded people and brands they believe in. We envision a world where only authentic brands and marketers win.
</p>
</div>
</section>
<section style={{background: '#fff',width: '100vw',marginLeft: 'calc(-50vw + 50%)',padding: '40px 20px 80px'}}>
<div style={{ maxWidth: '1200px', margin: '0 auto', textAlign: 'center' }}>
<img src="/authentic.png" alt="Authenticity" style={{width: '220px',height: 'auto',margin: '0 auto 32px'}} />
