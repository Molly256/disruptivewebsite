...
### *Do this for the other 2 API routes too*
Add the same line to the top of:
1. `app/api/user/route.js`
2. `app/api/user/search/route.js`
export const dynamic = 'force-dynamic'
### *Why*
Next.js 14 tries to prerender API routes by default. As soon as you use `req.url` it becomes dynamic. `export const dynamic = 'force-dynamic'` tells Next: "don't prerender this, run it on every request"

After adding this to all 3 files:
git add .
git commit -m "fix: force dynamic on api routes"
git push
Build should pass now ✅import { NextResponse } from 'next/server'
import fs from 'fs'
import path from 'path'
