...
export default function Sidebar() {
  ...
  if (!logged && isSupportPage) return null
  ...
  return <>
    ...
    <header style={{
      ...
      <img 
        ...
        alt="Disruptive" 
        onClick={() => go('/')}
        style={{height:22, width:'auto', cursor:'pointer', display:'block'}}
      />
      ...
      {simple? (
        ...
        <button className="mobile-hamburger" onClick={() => setOpen(!open)} style={{background:'none',border:'none',cursor:'pointer',padding:0}}>
          {open ? (
            <svg width="24" height="24" viewBox="0 0 24 24" stroke="#000" strokeWidth="2"><path d="M6 6l12 12M6 18L18 6"/></svg>
          ) : (
    ...
    {!simple && open && (
      <div style={{position:'fixed',top:56,left:0,right:0,bottom:0,background:'#fff',zIndex:1001,overflowY:'auto',padding:'40px 20px'}}>
        ...
        {home && <button onClick={() => go('/registration')} style={{background:'#cc0000',color:'#fff',border:'none',padding:'16px 32px',fontWeight:500,fontSize:16,letterSpacing:1,marginTop:30,cursor:'pointer',borderRadius:4}}>GET STARTED</button>}
      </div>
    )}

    {home && <div onClick={() => go('/registration')} onMouseDown={drag} style={{position:'fixed',bottom:20,right:20,zIndex:1000,background:'#e60000',color:'#fff',fontWeight:500,fontSize:14,letterSpacing:1,padding:'16px 24px',borderRadius:50,cursor:'grab',boxShadow:'0 4px 12px rgba(0,0,0,0.3)',userSelect:'none',touchAction:'none'}}>GET STARTED</div>}
  </>
}